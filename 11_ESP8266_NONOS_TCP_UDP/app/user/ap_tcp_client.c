...
void ESP8266_NetCon_Init(void)
{
	...
	//u8 remote_ip[4] = {192.168.4.2};				//设置Server(手机、电脑网络调试助手等设备)IP地址
...
void ICACHE_FLASH_ATTR AP_TCP_Client_Init(void)
{
	os_printf("\r\n=============================================================\r\n");
	...
	/* OLED屏初始化 */
	OLED_Init();								// OLED屏初始化
	OLED_ShowString(0,0,"ESP8266 = Client");	// ESP8266作为Client
	OLED_ShowString(0,2,"IP:");					// ESP8266_IP地址
	OLED_ShowString(0,4,"Remote  = Server");	// 网络调试助手作为Server
	OLED_ShowString(0,6,"IP:");					// 远端主机IP地址

	/* 软件定时器 */
	OS_Timer_1_Init(30000, 1);				// 软件定时器初始化
}
