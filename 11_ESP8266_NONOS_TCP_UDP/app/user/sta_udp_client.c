...
void ESP8266_NetCon_Init(void)
{
	...
	// 此处需要设置目标(Server)IP/端口(ESP8266作为Client，需要预先知道Server(网络调试助手等设备)的IP/端口)
	ST_NetCon.proto.udp->local_port  = 8266;		// 设置本地 Client(8266)端口
	ST_NetCon.proto.udp->remote_port = 8888;		// 设置目标 Server(网络调试助手等设备)端口  远端主机
	...
	//u8 remote_ip[4] = {192.168.0.109};			//设置Server(网络调试助手等设备)IP地址
...
void ICACHE_FLASH_ATTR STA_UDP_Client_Init(void)
{
	os_printf("\r\n=============================================================\r\n");
